function [ output_args ] = plotDiffImages( Images, windowHeight, result )
...
%         line([ax1.Children(end).XLim(1),ax1.Children(end).XLim(2)],[i,i], 'Color', [0 230 0]./255, 'Linewidth',2);
    ...
    if isempty(textFields_Diff)
        ...
        for i=1:nbWindows
            color = [0 0.5 0];
            if i<nbWindows && sum((result(imageNb).differences_v(:,i) - result(imageNb).differences_v(:,i+1)) == 0) ~= 2
                color = [1 0 0];
            elseif i>1 && sum((result(imageNb).differences_v(:,i) - result(imageNb).differences_v(:,i-1)) == 0) ~= 2
                color = [1 0 0];
            end
            textFields_Diff(i) = uicontrol('style', 'text', 'String', [int2str(result(imageNb).differences_v(2,i) ) ...
                ' - ' int2str(result(imageNb).differences_v(1,i))], 'Units', 'normalized',...
...
%             textFields_CorrDelta(i) = uicontrol('style', 'text', 'String', num2str(result(imageNb).corrCoeffs_y(i)), ...
        ...
        for i=1:nbWindows
            ...
            set(textFields_Diff(i), 'String', [int2str(result(imageNb).differences_v(2,i) ) ' - ' int2str(result(imageNb).differences_v(1,i))]);
