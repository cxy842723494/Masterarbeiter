function [phase_0_center nbDiffPicsFor2Pi] = powerSweep_halfNbFrames(sweepFreq, plain_width)
...
%     numFrames = 60/abs(sweepFreq-60); % Might be more precise than this
    numFrames = 62;
    nbDiffPicsFor2Pi = 61;
...
%     diff_G = single(SweepsGPU(1:2:end, 1:2:end,1:2:end)) - single(SweepsGPU(1:2:end, 1:2:end,2:2:end));
    ...
    P_B_2 = zeros(1,size(diff_B_2,3),'single', 'gpuArray');
%     P_G = zeros(1,size(diff_G,3),'single', 'gpuArray');
    ...
    P_B_united = [P_B P_B_2];
%     P_G(1:length(P_G)) = sum(sum(diff_G.^2,1),2);
    
    
    
    [P_B_ext, plain, phase_opp_1] = david.findPlain(P_B_united, plain_width);
...
%     phase_0_center = phase_0_raw + round(plain_width/2);
    ...
    if phase_0_center > nbDiffPicsFor2Pi + 2
...
%     figure;
    plot(P_B_united);
    ax = gca;
    line([phase_opp_1_center phase_opp_1_center],[ax.YLim(1),ax.YLim(2)], 'Linewidth', 1, 'Color', [.7 0 0], 'Linestyle', '--');
    line([phase_opp_2_center phase_opp_2_center],[ax.YLim(1),ax.YLim(2)],'Linewidth', 1,  'Color', [.7 0 0], 'Linestyle', '--');
%}
